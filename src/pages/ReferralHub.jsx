...
import {
  Users, DollarSign, Copy, TrendingUp, Share2, Link as LinkIcon,
  CheckCircle2, Clock, Trophy, Award, Zap, Star, Twitter, 
  Facebook, MessageCircle, Loader2, ChevronRight, Shield, Crown
} from 'lucide-react';
...
export default function ReferralHub() {
  ...
  const tier1Min = 0; const tier2Min = 10; const tier3Min = 50;
  const currentTier = activeReferrals >= tier3Min ? 3 : activeReferrals >= tier2Min ? 2 : 1;
  const nextTierTarget = currentTier === 1 ? tier2Min : currentTier === 2 ? tier3Min : tier3Min;
  const tierProgress = currentTier < 3
    ? Math.min(((activeReferrals - (currentTier === 1 ? 0 : tier2Min)) / (nextTierTarget - (currentTier === 1 ? 0 : tier2Min))) * 100, 100)
    : 100;
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Tabs defaultValue="link">
          ...
          <TabsContent value="progress" className="space-y-4 mt-4">
            {[
              {
                tier: 1, name: 'BitLabs Tier', min: 0, max: 10, color: 'bg-blue-500',
                desc: 'Standard survey revenue. Earn 50% of every survey. Referral commission: 25%.',
                perks: ['BitLabs Survey Access', '25% Referral Commission', '$1 Bonus per new referral'],
                earning: 'Up to ~$1,095/yr'
              },
              {
                tier: 2, name: 'PPC Network Tier', min: 10, max: 50, color: 'bg-purple-500',
                desc: '10 active referrals required. Answer 10 questions/day in our proprietary PPC network.',
                perks: ['PPC Question Network', 'Higher CPM rates', 'Priority Survey Queue'],
                earning: 'Up to ~$14,600/yr'
              },
              {
                tier: 3, name: 'Advertising Partner', min: 50, max: 50, color: 'bg-yellow-500',
                desc: '50+ active referrals. High-volume advertising partnerships and maximum commissions.',
                perks: ['Ad Partnership Access', 'Max Commission Rates', 'Dedicated Account Manager'],
                earning: '$58,400+/yr'
              }
            ].map((t) => {
              const isUnlocked = activeReferrals >= t.min || t.tier === 1;
              const isCurrent = currentTier === t.tier;
              const progress = t.tier < 3
                ? Math.min(((activeReferrals - t.min) / (t.max - t.min)) * 100, 100)
                : 100;
                    ...
                    {referrals.map((referral) => (
                      <div key={referral.id} className="flex items-center justify-between border rounded-xl p-4 bg-gray-50 hover:bg-white transition-colors">
                        ...
                        <div className="text-right">
                          <Badge variant={referral.status === 'active' ? 'default' : 'secondary'} className="text-xs mb-1">
                            {referral.status}
                          </Badge>
