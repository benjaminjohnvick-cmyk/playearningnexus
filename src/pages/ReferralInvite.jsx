...
function openUrlFor(platform, text, link) {
  ...
  switch (platform) {
    ...
    case 'reddit': return `https://www.reddit.com/submit?title=${encodeURIComponent('Check out Get Goods Gratis (Free)')}&text=${t}`;
...
export default function ReferralInvite() {
  ...
  const shareSocial = async () => {
    ...
    try {
      if (navigator.share) { await navigator.share({ title: 'Get Goods Gratis (Free)', text, url: link }); setOpenedCount((n) => n + 1); }
