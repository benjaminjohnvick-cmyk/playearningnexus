        ...
        <div ref={printRef}>
          ...
          <div className="p-8 space-y-6">
            ...
            <div style={{ marginTop: 24, padding: '12px 16px', backgroundColor: '#f9fafb', borderRadius: 8, fontSize: 11, color: '#888', textAlign: 'center' }}>
              This is a tax-compliant document generated by Get Goods Gratis (Free) Ad Grid. Invoice #{data.invoiceNumber}
            </div>
          </div>
        </div>
...
export default function AdPDFReportCenter({ userId, userEmail, ads }) {
  ...
  const handleEmail = async () => {
    ...
    const body = `Hi,\n\nPlease find your Get Goods Gratis (Free) ${reportType === 'invoice' ? 'Tax Invoice' : 'Performance Report'} for the period ${dateFrom} to ${dateTo}.\n\n📊 SUMMARY\n• Total Ad Spend: $${d.totalSpend.toFixed(2)}\n• Total Top-Ups: $${d.totalDeposits.toFixed(2)}\n${d.taxRate > 0 ? `• VAT (${(d.taxRate * 100).toFixed(0)}%): $${d.taxAmount.toFixed(2)}\n` : ''}• Total Billable: $${d.grandTotal.toFixed(2)}\n• Transactions: ${d.filtered.length}\n• Invoice #: ${d.invoiceNumber}\n\n📈 CAMPAIGNS\n${ads.map(ad => `• ${ad.brand_name}: ${ad.surveys_completed || 0} completions, $${(ad.total_spent || 0).toFixed(2)} spent`).join('\n')}\n\nBest regards,\n${branding.companyName || 'Get Goods Gratis (Free) Ad Grid'}`;
    await base44.integrations.Core.SendEmail({ to: recipientEmail, subject: `Get Goods Gratis (Free) Report #${d.invoiceNumber} — ${dateFrom} to ${dateTo}`, body, from_name: branding.companyName || 'Get Goods Gratis (Free) Ad Grid' });
    setSending(false);
