        ...
        <div ref={printRef}>
          ...
          <div className="p-8 rounded-t-2xl" style={{ backgroundColor: branding.color + '15', borderBottom: `4px solid ${branding.color}` }}>
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                <p className="font-black text-gray-900 text-lg">{branding.companyName || 'Grandia Granaria Ad Grid'}</p>
          ...
          <div className="p-8 space-y-6">
            ...
            <div style={{ marginTop: 24, padding: '12px 16px', backgroundColor: '#f9fafb', borderRadius: 8, fontSize: 11, color: '#888', textAlign: 'center' }}>
              This is a tax-compliant document generated by Grandia Granaria Ad Grid. Invoice #{data.invoiceNumber}
            </div>
          </div>
        </div>
...
export default function AdPDFReportCenter({ userId, userEmail, ads }) {
  ...
  const handleEmail = async () => {
    ...
    const body = `Hi,\n\nPlease find your Grandia Granaria ${reportType === 'invoice' ? 'Tax Invoice' : 'Performance Report'} for the period ${dateFrom} to ${dateTo}.\n\n📊 SUMMARY\n• Total Ad Spend: $${d.totalSpend.toFixed(2)}\n• Total Top-Ups: $${d.totalDeposits.toFixed(2)}\n${d.taxRate > 0 ? `• VAT (${(d.taxRate * 100).toFixed(0)}%): $${d.taxAmount.toFixed(2)}\n` : ''}• Total Billable: $${d.grandTotal.toFixed(2)}\n• Transactions: ${d.filtered.length}\n• Invoice #: ${d.invoiceNumber}\n\n📈 CAMPAIGNS\n${ads.map(ad => `• ${ad.brand_name}: ${ad.surveys_completed || 0} completions, $${(ad.total_spent || 0).toFixed(2)} spent`).join('\n')}\n\nBest regards,\n${branding.companyName || 'Grandia Granaria Ad Grid'}`;
    await base44.integrations.Core.SendEmail({ to: recipientEmail, subject: `Grandia Granaria Report #${d.invoiceNumber} — ${dateFrom} to ${dateTo}`, body, from_name: branding.companyName || 'Grandia Granaria Ad Grid' });
    setSending(false);
