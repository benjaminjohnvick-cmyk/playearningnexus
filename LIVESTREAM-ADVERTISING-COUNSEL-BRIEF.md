...
- **Moderation & DMCA (updated):** we have now BUILT an AI moderation layer (rules-first + AI classification of titles/products/messages, a kill-switch on a block, viewer reporting with auto-suspend, and a repeat-infringer strike system) **and** wired our existing notice-and-takedown (`dmcaTakedownRequest`) to strike hosts. We understand AI moderation does **not** replace the registered DMCA agent. So the precise question is: **is "AI moderation as the moderation layer + a registered DMCA designated agent + notice-and-takedown + an automated repeat-infringer termination policy" sufficient for safe harbor at broadcast scale?** What, if anything, is still missing? (See `AI-MODERATION-AND-DMCA.md`.) Please also confirm the mechanics of registering the designated agent with the U.S. Copyright Office and the required policy postings.
