...
// Daily: refresh personalized UserRecommendation records based on activity — ROUTED THROUGH THE AUTONOMY KERNEL
// (personalization_home domain). The whole pass auto-runs once that domain has earned autonomy AND the global
// live gate is open; otherwise it's queued for the overseer (nothing is written this run). Reversible.
export default __handler(async (req) => {
  const base44 = createClientFromRequest(req);

  try {
    const gate = await gateAndRun("personalization_home",
      { summary: "Refresh personalized recommendations for recent users", reversible: true },
      async () => {
...
Return 3 recommendations as array of {title, description, category, cta_url}`,
    ...
    if (!gate.executed) return Response.json({ ok: true, updated: 0, queued: gate.pending, gate_reason: gate.reason });
    return Response.json({ ok: true, updated: gate.result ?? 0 });
  } catch (err) {
