...
// closed-loop) or by CARD (adds the platform markup). The seller ships; an Order is opened for the
// AI-managed fulfillment lifecycle (escrow → seller ships → funds release).
...
export default __handler(async (req) => {
  try {
    ...
    // Validate the seller BEFORE taking any money — never debit a buyer for an un-shippable item.
    const [seller] = await base44.asServiceRole.entities.User.filter({ id: listing.seller_id });
    ...
    // Open an Order for AI-managed fulfillment (seller ships).
    const order = await base44.asServiceRole.entities.Order.create({
      ...
      fulfillment_type: "seller_ship",
      shipping_address: shipping_address || null,
      status: "awaiting_shipment",
      created_at: new Date().toISOString(),
    });

    // Notify both sides; the AI fulfillment lifecycle (autoOrderFulfillmentAndFundsRelease) takes over.
    ...
    await base44.asServiceRole.entities.Notification.create({
      ...
      title: "🛍️ Purchase confirmed", message: `You bought "${listing.title}". The seller will ship it soon.`, is_read: false,
    }).catch(() => null);
    base44.asServiceRole.functions.invoke("autoOrderFulfillmentAndFundsRelease", { order_id: (order as any).id }).catch(() => null);

    return Response.json({ success: true, order_id: (order as any).id, charged });
  } catch (error) {
