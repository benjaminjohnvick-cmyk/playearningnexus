...
export const REGISTRY: SettingDef[] = [
  ...
  { key: "REFERRAL_SIGNUP_BONUS_USD", label: "Referral: activation bonus", category: "Referrals", type: "number", default: "4", unit: "$", help: "Bonus (in non-cashable points) paid to the referrer when their referred user completes a first fraud-screened survey. Platform-funded. Paid once per referral.", min: 0 },
  ...
  { key: "PROMO_FUNDED_BY_MARKUP", label: "Welcome credit funded by markup (margin-positive)", category: "Marketplace", type: "boolean", default: "1", help: "ON = the welcome/promotional discount applied to any order is capped at that order's markup, so the sale's net never drops below the base price — the markup pays for the promo and every sale stays margin-positive. OFF = the welcome discount uses its full per-order cap (can exceed the markup; funded by breakage instead)." },
