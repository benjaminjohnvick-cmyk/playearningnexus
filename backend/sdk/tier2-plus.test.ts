// tier2-plus.test.ts — unit tests for the Tier 2 Plus uncapped-scaling quote. Run in the Deno backend:
//   deno test backend/sdk/tier2-plus.test.ts
...
Deno.test("doubling the budget doubles deliverables, value, and guaranteed impressions", () => {
  const a = tier2PlusQuote(200000);
  const b = tier2PlusQuote(400000);
...
Deno.test("delivery is always capacity-paced and prepaid (never credit)", () => {
  const q = tier2PlusQuote(1000000);
