...
> and feature is included in the provisional as filed** to maximize written-description support for the broadest
> possible later claims. Please keep Appendix A attached to the provisional when filing (and confirm whether any
> item should instead be held as trade secret per §11 before the non-provisional publishes).
...
functions across the entire platform, not a representative subset. Representative modules are also named per
subsystem in Section 6. (Appendix A is included in this provisional rather than merely incorporated by reference,
so the full disclosure receives this application's priority date.)
...
13. **File the complete functions-and-features inventory (Appendix A) WITH the provisional.** The owner's
    intent is that **every function and feature be included in the provisional as filed** (not merely referenced),
    so the full disclosure receives the priority date and supports the broadest later claims. Confirm Appendix A
...
> **For patent counsel — novelty & claim-scope assessment (added 2026-09-11).** This is a complete feature
> disclosure for counsel to evaluate, **not** a novelty opinion; patent-eligibility, prior-art clearance, and
> claim scope are counsel's to determine. Recommend a targeted prior-art search on watch-gated / rewarded survey
> flows before any filing, and — if pursued — a provisional before the interaction is shown publicly.
